...
function Perlin() {
  ...
  useEffect(() => {
    ...
    material.onBeforeCompile = (
      ...
      webglProgram.uniforms.uPerlinTime = { value: 1.0 };

      if (webglProgram.vertexShader.includes("cnoise4D")) {
...
float cnoise4D(vec4 P){
  ...
  return 2.2 * n_xyzw;
      ...
      webglProgram.vertexShader = webglProgram.vertexShader.replace(
        ...
        float vPerlinDisplacement = 1.3 * cnoise4D(vec4(objectNormal, uPerlinTime * 0.2) * 2.1) + 2.1;
        `
      );
      webglProgram.vertexShader = webglProgram.vertexShader.replace(
        ...
        transformed.xyz = transformed.xyz + vPerlinDisplacement * objectNormal.xyz;
  ...
  useFrame((state, delta) => {
    if (transmissionShaderRef.current) {
      transmissionShaderRef.current.uniforms.uPerlinTime.value += delta;
    }
  });
